...
class IslesAllSlicesDataset(Dataset):
    def __init__(self, data_root, list_path, img_size=224):
        self.data_root = data_root
        self.case_list = load_case_list(list_path)
        self.img_size = img_size
        self.samples = [] 
        ...
        for case_name in self.case_list:
            ...
            if os.path.exists(file_path):
                try:
                    ...
                    dwi = data['dwi']
                    # logic to check all slices
                    valid_slices = []
                    for i in range(dwi.shape[0]):
                         if np.any(dwi[i]):
                             valid_slices.append((file_path, i))
                    self.samples.extend(valid_slices)
                except:
                    pass
    def __len__(self):
